...
async def main():
    async with async_playwright() as p:
        ...
        await page.fill("input[placeholder='Max Mustermann']", username) # "Anzeigename" might not be exactly "input[type='text']", using placeholder
        # Wait, the screenshots earlier showed input type=text is present. Let's just use placeholder to be safe.
        ...
        print("Click login")
        # Notice we don't have to fill login again in this app, it might auto fill. But let's fill it if needed.
        # Actually from screenshots, it just shows the login form.
        # But wait, looking at the previous screenshot, it showed "Registrierung erfolgreich! Bitte logge dich ein." and the login form was pre-filled or we can fill it.
        await page.fill("input[type='email']", username + "@example.com")
        await page.fill("input[type='password']", "password")
        await page.click("button:has-text('Einloggen')")

        # explicitly wait for an element in the home page like 'Liste erstellen / beitreten'
